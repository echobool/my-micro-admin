<template>
  <div class="app-container">
    <el-row :gutter="10" type="type">
      <el-col :xs="22" :sm="22" :md="22" :lg="23" :xl="23">
        ...
        <el-form ref="searchForm" :inline="true" :model="searchForm" :rules="rules" class="search">
          <el-form-item>
            <el-button type="primary" round :disabled="!checkPermission(['CreateSmsTemplate'])" icon="el-icon-edit" @click="$router.push({name: 'CreateSmsTemplate'})">添加模板</el-button>
          </el-form-item>
        </el-form>
      </el-col>
      <!--刷新-->
    ...
    <el-table v-loading="listLoading" :highlight-current-row="true" :data="list" stripe fit style="width: 100%">
      ...
      <el-table-column class-name="status-col" label="模板ID">
        <template slot-scope="scope">
          <span>{{ scope.row.template_code }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="status-col" label="传参案例">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="操作" width="180">
        <template slot-scope="scope">
          ...
          <el-button plain type="primary" :disabled="!checkPermission(['EditSmsTemplate'])" size="mini" style="margin-left:10px;" @click="$router.push({name: 'EditSmsTemplate',params: {id:scope.row.id}})">
            编辑
          </el-button>

          <el-button plain type="danger" :disabled="!checkPermission(['DeleteSmsTemplate'])" size="mini" style="margin-left:10px;" @click="deleteConfirm(scope.row)">
            删除
          </el-button>
...
<script>
import { fetchSmsTemplateList, deleteSmsTemplate } from '@/api/message'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      fetchSmsTemplateList(this.listQuery).then(response => {
        this.list = response.data.SmsTemplates
    ...
    deleteConfirm(row) {
      this.$confirm('您确认要删除 ' + row.name + ' 该模板吗? 可在数据库中恢复。', '是否继续?', {
        ...
        // 进行远程操作
        deleteSmsTemplate(row.id).then(response => {
          this.getList()
          this.$message({
            type: 'success',
            message: '删除成功!'
